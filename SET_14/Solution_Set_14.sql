...
select first_name, last_name , salary from employees
where salary > (select avg(salary) from employees) And department_id in (select department_id from departments where department_name = 'IT')
...
select employee_id, first_name, last_name ,department_name salary from employees a
join departments on departments.department_id = a.department_id
...
select salary from Employees ORDER BY `salary` DESC limit 5,1;
